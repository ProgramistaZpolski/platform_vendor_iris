...
func init() {
	android.RegisterModuleType("iris_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = irisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = irisExpandVariables(ctx, dep_file)
	...
	cmd := irisExpandVariables(ctx, String(g.properties.Cmd))
